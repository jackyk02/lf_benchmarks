...
class __envreactor:
    ...
    def reaction_function_1(self, seed, infos):
        val = copy.deepcopy(seed.value)
        start_time = time.time()
        policy = np.random.default_rng(val)
        ...
        self.env.reset(seed=123, options={})
        infos.set((observations, rewards, terminations, np.ones(1280000)))
        end_time = time.time()
...
class __serverreactor:
    ...
    def reaction_function_1(self, infos, seed):
        ...
        for i in range(16):
            observations = copy.deepcopy(infos[i].value[0])
            print(f"Env {i + 1}: Number of Observations = {len(observations)}")
        print("\n")

        if self.episode_num == NUM_EPISODES-1:
            end_time = time.time()
...
sample_batch_client_lf = [None] * 16
...
for sample_batch_main_i in range(1):
    ...
    for sample_batch_client_i in range(16):
